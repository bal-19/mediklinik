...
export function renderApp(pathname: string) {
  if (pathname.startsWith('/klinik/nonaktif')) {
    return renderInactiveClinicPage();
  }
  ...
  if (pathname.startsWith('/app')) {
    return renderDashboardShell(pathname);
  }
...
function renderInactiveClinicPage() {
  return `
    <div class="flex min-h-screen items-center justify-center bg-slate-50 px-6">
      <div class="w-full max-w-2xl rounded-[28px] border border-slate-200 bg-white p-8 text-center shadow-xl shadow-slate-200/60">
        <div class="text-sm font-semibold uppercase tracking-[0.2em] text-slate-500">Klinik Tidak Aktif</div>
        <h1 class="mt-4 text-4xl font-bold">Klinik ini sedang tidak tersedia.</h1>
        <p class="mt-4 text-base leading-8 text-slate-600">Subscription klinik sedang berstatus EXPIRED atau SUSPENDED, sehingga halaman publik dan dashboard dibatasi sementara.</p>
        <a class="mt-8 inline-flex rounded-full bg-sky-500 px-6 py-3 text-sm font-semibold text-white" href="/">Kembali ke MediKlinik</a>
      </div>
    </div>
  `;
}
...
function renderDashboardShell(pathname: string) {
  if (pathname.startsWith('/app/billing/blocked')) {
    return renderBillingBlockedPage();
  }

  const mainContent = renderDashboardContent(pathname);
...
function renderDashboardContent(pathname: string) {
  if (pathname.startsWith('/app/queues')) {
    return `
      <section class="grid gap-6 lg:grid-cols-3">
        ${dashboardSnapshot.queueItems
          .map(
            (item) => `
            <article class="rounded-3xl border border-slate-200 bg-white p-6">
              <div class="text-sm text-slate-500">${item.patient}</div>
              <div class="mt-3 font-mono text-4xl font-bold">${item.queueNumber}</div>
              <div class="mt-4 text-sm font-semibold text-sky-700">${item.status}</div>
            </article>`,
          )
          .join('')}
      </section>
          ...
          .map(
            ...
            <article class="rounded-3xl border border-slate-200 bg-white p-6">
              <div class="flex items-center justify-between">
                ...
                } px-3 py-1 text-xs font-semibold">${item.unit}</span>
              </div>
              <p class="mt-4 text-sm text-slate-600">Stok ${item.stockQuantity} • Minimum alert ${item.minStockAlert}</p>
            </article>`,
          )
          .join('')}
      </section>
        ...
        ${dashboardSnapshot.invoices
          .map(
            ...
            <article class="rounded-3xl border border-slate-200 bg-white p-6">
              <div class="flex items-center justify-between">
                <div>
                  <div class="text-sm text-slate-500">${item.id}</div>
                  <h2 class="mt-2 text-xl font-semibold">${item.patient}</h2>
                </div>
                <span class="rounded-full ${
                  item.status === 'PAID' ? 'bg-sky-100 text-sky-700' : 'bg-amber-100 text-amber-700'
                } px-3 py-1 text-xs font-semibold">${item.status}</span>
              </div>
              <div class="mt-4 text-2xl font-bold">Rp ${item.amount.toLocaleString('id-ID')}</div>
            </article>`,
          )
          .join('')}
      </section>
    ...
    return `
      <section class="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        <article class="rounded-3xl border border-amber-200 bg-amber-50 p-6">
          <div class="text-sm font-semibold text-amber-700">Status Subscription</div>
          <div class="mt-3 text-3xl font-bold">TRIAL • ${dashboardSnapshot.daysRemaining} hari tersisa</div>
          <p class="mt-4 text-sm leading-7 text-slate-700">Semua fitur masih aktif. Admin klinik dapat setup Midtrans klinik dan memperpanjang paket sebelum masa trial berakhir.</p>
          <div class="mt-6 flex gap-4">
            <a class="rounded-full bg-sky-500 px-5 py-3 text-sm font-semibold text-white" href="/app/billing/checkout">Perpanjang Sekarang</a>
            <a class="rounded-full border border-slate-300 bg-white px-5 py-3 text-sm font-semibold text-slate-700" href="/app/settings/midtrans">Setup Midtrans</a>
          </div>
        </article>
        <article class="rounded-3xl border border-slate-200 bg-white p-6">
          <div class="text-sm text-slate-500">Riwayat terakhir</div>
          <div class="mt-3 text-lg font-semibold">Belum ada pembayaran subscription</div>
          <p class="mt-3 text-sm leading-7 text-slate-600">Saat trial selesai, histori pembayaran platform Midtrans akan muncul di area ini.</p>
        </article>
      </section>
    `;
  }
      ...
      <section class="grid gap-6">
        ${dashboardSnapshot.records
          .map(
            (item) => `
            <article class="rounded-3xl border border-slate-200 bg-white p-6">
              <h2 class="text-xl font-semibold">${item.patient}</h2>
              <p class="mt-3 text-sm text-slate-600">Diagnosa terakhir: ${item.diagnosis}</p>
              <p class="mt-2 text-sm text-slate-500">Dokter: ${item.doctor}</p>
            </article>`,
          )
          .join('')}
      </section>
  ...
  return `
    <section class="grid gap-6 lg:grid-cols-3">
      <article class="rounded-3xl border border-slate-200 bg-white p-6"><div class="text-sm text-slate-500">Antrian hari ini</div><div class="mt-3 font-mono text-4xl font-bold">${dashboardSnapshot.todayQueueNumber}</div></article>
      <article class="rounded-3xl border border-slate-200 bg-white p-6"><div class="text-sm text-slate-500">Pendapatan hari ini</div><div class="mt-3 text-4xl font-bold">Rp ${dashboardSnapshot.todayRevenue.toLocaleString('id-ID')}</div></article>
    ...
    <section class="mt-6 grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
      <article class="rounded-3xl border border-slate-200 bg-white p-6">
        <div class="text-sm text-slate-500">Alert stok menipis</div>
        <div class="mt-5 space-y-4">
          ${dashboardSnapshot.lowStockAlerts
            .map(
              (item) => `
              <div class="rounded-2xl bg-amber-50 p-4">
                <div class="font-semibold">${item.medicineName}</div>
                <div class="mt-2 text-sm text-slate-600">Sisa ${item.stockQuantity}, minimum ${item.minStockAlert}</div>
              </div>`,
            )
            .join('')}
        </div>
      ...
      <article class="rounded-3xl border border-slate-200 bg-white p-6">
        <div class="text-sm text-slate-500">Snapshot laporan</div>
        <div class="mt-5 text-sm leading-8 text-slate-700">
          <div>Kunjungan: ${dashboardSnapshot.reports.visits.join(' • ')}</div>
          <div class="mt-4">Pendapatan: ${dashboardSnapshot.reports.revenue.join(' • ')}</div>
        </div>
      </article>
    </section>
  `;
...
function renderBillingBlockedPage() {
  return `
    <div class="flex min-h-screen items-center justify-center bg-slate-50 px-6">
      <div class="w-full max-w-2xl rounded-[28px] border border-slate-200 bg-white p-8 shadow-xl shadow-slate-200/60">
        <div class="rounded-full bg-amber-100 px-4 py-2 text-sm font-semibold text-amber-700">Subscription EXPIRED</div>
        <h1 class="mt-6 text-4xl font-bold">Masa langganan Anda telah berakhir.</h1>
        <p class="mt-4 text-base leading-8 text-slate-600">Akses ke area dashboard dibatasi sampai pembayaran subscription MediKlinik diperpanjang. Data klinik tetap aman dan dapat diaktifkan kembali setelah pembayaran sukses.</p>
        <div class="mt-8 flex gap-4">
          <a class="rounded-full bg-sky-500 px-6 py-3 text-sm font-semibold text-white" href="/app/billing">Buka Halaman Billing</a>
          <a class="rounded-full border border-slate-300 bg-white px-6 py-3 text-sm font-semibold text-slate-700" href="/">Kembali ke Landing</a>
        </div>
      </div>
    </div>
  `;
